...
class DataLoader:
    ...
    def download_river_data(self):
        ...
        try:
            ...
            logger.warning(f"直接讀取失敗: {e}")
            
            # 方法2: 手動下載 ZIP 檔案
            logger.info("嘗試方法2: 手動下載 ZIP 檔案...")
            ...
            try:
                ...
                if response.status_code == 200:
                    # 保存 ZIP 檔案
                    zip_path = self.data_dir / "RIVERPOLY.zip"
                    with open(zip_path, 'wb') as f:
                        f.write(response.content)
                    
                    logger.info(f"ZIP 檔案已下載: {zip_path}")
                    ...
                    if shp_files:
                        ...
                        logger.info(f"找到 Shapefile: {shp_path}")
