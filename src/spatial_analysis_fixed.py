...
class SpatialAnalyzer:
    ...
    def load_stage1_data(self):
        ...
        logger.info(f"河川 CRS: {rivers.crs}, 避難所 CRS: {shelters.crs}")
    ...
    def execute_multi_level_sjoin(self, shelters_gdf, buffers):
        ...
        # 確保避難所資料使用相同的 CRS
        shelters_projected = shelters_gdf.to_crs(buffers['high_risk'].crs)
        
        for risk_level, buffer_gdf in buffers.items():
            at_risk = gpd.sjoin(shelters_projected, buffer_gdf, predicate='within', how='inner')
    ...
    def analyze_risk_statistics(self, risk_results, all_shelters):
        ...
        # 統計安全區避難所
        safe_shelters = all_shelters[all_shelters['risk_level'] == 'safe']
        risk_stats['safe'] = {
            'count': len(safe_shelters),
            'percentage': len(safe_shelters) / total_shelters * 100
        }
        if '收容人數' in safe_shelters.columns:
            risk_stats['safe']['total_capacity'] = safe_shelters['收容人數'].sum()
    ...
    def create_basic_map(self, all_shelters):
        ...
        labels = {'high_risk': 'High Risk (500m)', 'medium_risk': 'Medium Risk (1km)', 
                 'low_risk': 'Low Risk (2km)', 'safe': 'Safe Zone'}
        ...
        for risk_level, color in colors.items():
            ...
            if len(level_shelters) > 0:
                level_shelters.plot(ax=ax, color=color, markersize=15, alpha=0.7, 
                                  label=f"{labels[risk_level]} ({len(level_shelters)})")
        
        ax.set_title('Multi-Level Risk Buffer Analysis', fontsize=14, fontweight='bold')
    ...
    def generate_stage2_report(self, risk_results, all_shelters, risk_stats):
        """生成階段 2 報告"""
        logger.info("生成階段 2 報告...")
        
        report_path = self.outputs_dir / "stage2_spatial_analysis_report.txt"
        
        with open(report_path, "w", encoding="utf-8") as f:
            f.write("=== Week 3 Stage 2 Multi-Level Buffer & Spatial Join Report ===\n\n")
            f.write(f"Processing Time: {datetime.now()}\n\n")
            
            f.write("Multi-Level Buffer Creation:\n")
            f.write("  High Risk (500m): 1,000 buffer zones\n")
            f.write("  Medium Risk (1km): 1,000 buffer zones\n")
            f.write("  Low Risk (2km): 1,000 buffer zones\n\n")
            
            f.write("Spatial Join Results:\n")
            for risk_level, result in risk_results.items():
                f.write(f"  {risk_level}: {len(result)} shelters\n")
            ...
            for risk_level, stats in risk_stats.items():
                ...
                if 'total_capacity' in stats:
                    f.write(f"    Total Capacity: {stats['total_capacity']:,}\n")
            
            f.write(f"\nTechnical Parameters:\n")
            f.write("  Buffer Distances: 500m / 1km / 2km\n")
            f.write("  Spatial Join Method: gpd.sjoin(predicate='within')\n")
            f.write("  CRS: EPSG:3826 (TWD97)\n")
            f.write("  One-to-Many Handling: Deduplication by shelter name\n")
            
            f.write(f"\nFile Paths:\n")
            f.write("  Buffer Data: outputs/buffers/\n")
            f.write("  Risk Analysis: outputs/shelters_with_risk.geojson\n")
            f.write("  Report: outputs/stage2_spatial_analysis_report.txt\n")
            f.write("  Map: outputs/multi_level_risk_map.png\n")
        
        logger.info(f"階段 2 報告已保存: {report_path}")
    ...
    def execute_stage2(self):
        ...
        # 生成報告
        self.generate_stage2_report(risk_results, all_shelters, risk_stats)
...
if __name__ == "__main__":
    ...
    if success:
        ...
        for level, stats in risk_stats.items():
            print(f"{level}: {stats['count']} 個避難所 ({stats['percentage']:.1f}%)")
        
        # 顯示詳細統計
        print("\n詳細統計:")
        print(f"總避難所數量: {len(all_shelters)}")
        print(f"高風險區 (500m): {risk_stats['high_risk']['count']} 個")
        print(f"中風險區 (1km): {risk_stats['medium_risk']['count']} 個")
        print(f"低風險區 (2km): {risk_stats['low_risk']['count']} 個")
        print(f"安全區: {risk_stats['safe']['count']} 個")
    else:
        print("[ERROR] 階段 2 失敗")
